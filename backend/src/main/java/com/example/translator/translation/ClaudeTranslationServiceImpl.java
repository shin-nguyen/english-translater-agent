...
@Service
@ConditionalOnProperty(prefix = "translation", name = "provider", havingValue = "anthropic")
public class ClaudeTranslationServiceImpl implements TranslationService {

    private static final Logger log = LoggerFactory.getLogger(ClaudeTranslationServiceImpl.class);
    ...
    public ClaudeTranslationServiceImpl(RestClient anthropicRestClient,
    ...
    public TranslateResponse translate(TranslateRequest request) {
        ...
        String rawResponse = callClaude(systemPrompt, request.text());
        try {
            return parseResponse(rawResponse);
        } catch (RuntimeException firstFailure) {
            log.warn("AI response failed JSON parsing, retrying once with stricter instructions: {}", firstFailure.getMessage());
            String retryResponse = callClaude(systemPrompt + RETRY_SUFFIX, request.text());
            try {
